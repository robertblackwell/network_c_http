...
static void new_message_callback(void* msgstream, MSG_REF new_msg, int error);
static void invoke_read_callback(MsgStreamRef msg_stream_ref, MSG_REF new_msg, int error);
...
void msg_stream_read(MsgStreamRef msg_stream, MsgReadCallback cb, void* arg)
{
    ...
    if (List_size(msg_stream->input_message_list) != 0) {
        MSG_REF m = List_remove_first(msg_stream->input_message_list);
...
static void tcp_read_callback(void* arg, int error)
{
    ...
    if(error == 0) {
        ...
        if (List_size(msg_stream_ref->input_message_list) != 0) {
            MSG_REF m = List_remove_first(msg_stream_ref->input_message_list);
