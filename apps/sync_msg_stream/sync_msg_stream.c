...
typedef struct SyncMsgStream_s {
    int fd;
    GenericMsgParserRef parser_ref;
    GenericMsgRef new_msg;
} SyncMsgStream, *SyncMsgStreamRef;

void new_msg_callback(void* arg, GenericMsgRef new_msg, int error)
{
...
SyncMsgStreamRef sync_msg_stream_new()
{
    ...
    s->fd = -1;
    s->parser_ref = generic_msg_parser_new(new_msg_callback, s);
    return s;
}
void sync_msg_stream_free(SyncMsgStreamRef s)
{
    generic_msg_parser_free(s->parser_ref);
...
{
    IOBufferRef iob = generic_msg_serialize(msgref);
...
GenericMsgRef sync_msg_stream_recv(SyncMsgStreamRef sync_stream)
{
    ...
    while(1) {
        ...
        generic_msg_parser_consume(sync_stream->parser_ref, iob);
        assert(IOBuffer_data_len(iob) == 0);
        if (sync_stream->new_msg != NULL) {
            GenericMsgRef tmp = sync_stream->new_msg;
