...
TmplServerRef g_sref;
void sig_handler(int signo)
{
    printf("tmpl_app.c signal handler \n");
    if ((signo == SIGINT) || (signo == SIGABRT)) {
        printf("received SIGINT or SIGABRT\n");
        tmpl_server_free(g_sref);
        g_sref = NULL;
        exit(0);
    }
}
...
//    g_sref = sref;
    ...
    for (int p = 0; p < nbr_processes; p++) {
        if ((child_pid = fork()) == 0) {
            tmpl_process_main(host, port, nbr_threads, nbr_connections_per_thread, nbr_roundtrips_per_connection);
            exit(0);
        }
    }
    while (wait(NULL) > 0);
