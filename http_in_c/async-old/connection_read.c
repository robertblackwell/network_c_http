...
void async_read_start(AsyncConnectionRef connection_ref)
{
    ...
    RBL_LOG_FMT("async_read_start socket: %d read_state: %d %s", connection_ref->socket, connection_ref->read_state, async_read_state_str(connection_ref->read_state));
...
static void postable_reader(RunloopRef reactor_ref, void* arg)
{
    ...
    RBL_CHECK_TAG(AsyncConnection_TAG, connection_ref)
    RBL_LOG_FMT("postable_reader socket: %d read_state: %d %s", connection_ref->socket, connection_ref->read_state, async_read_state_str(connection_ref->read_state));
    RBL_ASSERT((connection_ref->read_state == READ_STATE_ACTIVE), "invalid state");
    RBL_ASSERT((connection_ref->readside_posted), "read_side posted should be true");
...
void reader(AsyncConnectionRef connection_ref) {
    RBL_CHECK_TAG(AsyncConnection_TAG, connection_ref)
    RBL_ASSERT((connection_ref->read_state == READ_STATE_ACTIVE), "invalid state");
    RBL_ASSERT((connection_ref->input_message_ptr == NULL), "input_message_ptr not null - invalid state");
    AsyncHandlerRef href = connection_ref->handler_ref;
    RBL_LOG_FMT("reader socket: %d read_state: %d %s", connection_ref->socket, connection_ref->read_state, async_read_state_str(connection_ref->read_state));
    ...
    RBL_LOG_FMT("bytes_available %d errno: %d %s", nread, errno_save, strerror(errno_save));
    int eagain = EAGAIN;
    char* errstr = strerror(errno_save);
    if(nread > 0) {
        RBL_LOG_FMT("Before AsyncParser_consume read_state %s", async_read_state_str(connection_ref->read_state));
        llhttp_errno_t ec = http_parser_consume(connection_ref->http_parser_ptr, IOBuffer_data(iob), IOBuffer_data_len(iob));
        RBL_LOG_FMT("After AsyncParser_consume returns  message_ptr: %p errno: %d errno_str: %s  read_state %s ",
        ...
        if(errno_save == EAGAIN) {
            ...
            RBL_LOG_FMT("test this result message_needs_eof %d", (int)x);
        ...
        RBL_LOG_FMT("bytes_available == 0 errno: %d %s", errno_save, strerror(errno_save))
        read_error(connection_ref, "reader zero bytes - peer closed connection");
    } else if (errno_save == eagain) {
        RBL_LOG_FMT("bytes_available < 0 eagain errno: %d %s", errno_save, strerror(errno_save))
        read_eagain(connection_ref);
    } else {
        RBL_LOG_FMT("bytes_available < 0 io-error errno: %d %s", errno_save, strerror(errno_save))
...
{
    RBL_CHECK_TAG(AsyncConnection_TAG, cref)
    RBL_ASSERT((cref->read_state == READ_STATE_ACTIVE), "invalid state");
    RBL_ASSERT((cref->input_message_ptr != NULL), "input_message_ptr null on new message");
    RBL_ASSERT((!cref->readside_posted), "nothing should be posted on readside at this point");
    RBL_LOG_FMT("socket: %d", cref->socket)
...
static void read_eagain(AsyncConnectionRef cref)
{
    RBL_CHECK_TAG(AsyncConnection_TAG, cref)
    RBL_ASSERT((cref->read_state == READ_STATE_ACTIVE), "invalid state");
    RBL_ASSERT((cref->input_message_ptr == NULL), "input_message_ptr NOT null should be handled by read_new_message");
    RBL_ASSERT((!cref->readside_posted), "nothing should be posted on readside at this point");
    RBL_LOG_FMT("read_eagain socket: %d connection_ref->read_state: %s", cref->socket, async_read_state_str(cref->read_state));
...
static void read_need_data(AsyncConnectionRef cref)
{
    RBL_CHECK_TAG(AsyncConnection_TAG, cref)
    RBL_ASSERT((cref->read_state == READ_STATE_ACTIVE), "invalid state");
    RBL_ASSERT((cref->input_message_ptr == NULL), "input_message_ptr NOT null on incomplete read");
    RBL_ASSERT((!cref->readside_posted), "nothing should be posted on readside at this point");
    RBL_LOG_FMT("read_need_data socket:%d connection_ref->read_state: %s", cref->socket, async_read_state_str(cref->read_state));
...
static void read_error(AsyncConnectionRef cref, char* msg)
{
    RBL_CHECK_TAG(AsyncConnection_TAG, cref)
    RBL_ASSERT((cref->read_state == READ_STATE_ACTIVE), "invalid state");
    RBL_ASSERT((cref->input_message_ptr == NULL), "input_message_ptr NOT null should not happen on read error");
    RBL_ASSERT((!cref->readside_posted), "nothing should be posted on readside at this point");
    RBL_LOG_FMT("socket: %d", cref->socket)
    RBL_LOG_FMT("Read_error got an error this is the message: %s  fd: %d", msg, cref->socket);
    AsyncHandlerRef href = cref->handler_ref;
    RBL_CHECK_TAG(AsyncHandler_TAG, href);
...
{
    RBL_CHECK_TAG(HTTP_PARSER_TAG, parser_ptr)
    ...
    RBL_ASSERT((msg != NULL), "msg should not be NULL");
    RBL_ASSERT((connection_ref->read_state == READ_STATE_ACTIVE), "must be read_state ACTIVE");
    RBL_LOG_FMT("socket: %d", connection_ref->socket)
...
static void postable_handle_request(RunloopRef reactor_ref, void* handler_ref_arg)
{
    ...
    AsyncConnectionRef cref = href->async_connection_ref;
    RBL_ASSERT((cref->scratch_request != NULL), "request should not be null");
