...
{
    IOBufferRef iob_body = HttpMessage_serialize(request);
...
{
    HttpMessageRef response = HttpMessage_new_response();
    ...
    CbufferRef target = HttpMessage_get_target_cbuffer(request);
    ...
    if(strcmp(target_cstr, "/echo") == 0) {
         ...
         */
        const char* echo_value = HttpMessage_get_header_value(request, HEADER_ECHO_ID);
        if(echo_value != NULL) {
            HttpMessage_add_header_cstring(response, HEADER_ECHO_ID, echo_value);
        }
    ...
    HttpMessage_set_status(response, HTTP_STATUS_OK);
    HttpMessage_set_reason(response, "OK");
    HttpMessage_add_header_cstring(response, HEADER_CONTENT_TYPE, "text/html; charset=UTF-8");

//    int cmp_tmp = HttpMessage_cmp_header(request, HEADER_CONNECTION_KEY, HEADER_CONNECTION_KEEPALIVE);
//    if(cmp_tmp == 1) {
//        HttpMessage_add_header_cstring(response, HEADER_CONNECTION_KEY, HEADER_CONNECTION_KEEPALIVE);
//    } else {
//        HttpMessage_add_header_cstring(response, HEADER_CONNECTION_KEY, HEADER_CONNECTION_CLOSE);
//    }

//    IOBufferRef request_serialized = HttpMessage_serialize(request);
//    IOBufferRef response_serialized = HttpMessage_serialize(response);
...
//    IOBuffer_free(response_serialized);
    HttpMessage_set_body(response, body_chain);
    HttpMessage_set_content_length(response, BufferChain_size(body_chain));
    return response;
}
