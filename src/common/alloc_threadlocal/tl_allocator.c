...
void tl_allocator_init(Tlocal_Allocator* tla)
{
    ...
    tl_freelist_init(tla->free_list);
    tla->allocated_list = &(tla->allocated_list_mem);
    tl_freelist_init(tla->allocated_list);
    ...
    tl_freelist_add(tla->free_list, blk);
}
...
{
    MBlock* block = tl_freelist_find_space(tl->free_list, size);
    if(block == NULL) {
        ...
        tl_freelist_remove(tl->free_list, block);
        RBL_ASSERT((block->forward == NULL), "new block invariant failed");
        RBL_ASSERT((block->backward == NULL), "new block invariant failed");
    }
    if(memblock_should_split(block, size)) {
        MBlock* remainder = memblock_split(block, size);
        tl_freelist_add(tl->free_list, remainder);
    }
    // memblock_mark_allocated(block);
    tl_freelist_add(tl->allocated_list, block);
    memblock_check_tags(block);
...
void  tl_allocator_free(Tlocal_Allocator* tl, void* ptr)
{
    ...
    tl_freelist_remove(tl->allocated_list, block);
    ...
    while(1) {
        MBlock* merge_block = tl_freelist_find_merge(tl->free_list, block);
        if(merge_block == NULL) {
            break;
        }
        tl_freelist_remove(tl->free_list, merge_block);
        block = memblock_merge(block, merge_block);
    }
    // memblock_mark_free(block);
    tl_freelist_add(tl->free_list, block);
}
...
{
    // tl_freelist_empty(tl->allocated_list);
    tl_freelist_empty(tl->free_list);
