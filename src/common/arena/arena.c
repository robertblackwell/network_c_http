...
UserMemory arena_user_ptr_from_allocation(AllocatedMemory* allocation)
{
...
void* arena_alloc(Arena* arena, size_t user_alloc_size)
{
    ...
    while(p != NULL) {
        if(arena_block_can_satisfy_alloc(p, user_alloc_size)) {
            AllocatedMemory* alloc_memptr = arena_block_alloc(p, user_alloc_size);
            ARENA_ALLOCATED_MEMORY_REDZONE_VERIFY(alloc_memptr)
    ...
    AllocatedMemory* alloc_memptr = arena_block_alloc(newptr, user_alloc_size);
    RBL_ASSERT((alloc_memptr != NULL), "allocator trying to return NULL");
    ARENA_ALLOCATED_MEMORY_REDZONE_VERIFY(alloc_memptr)
    return arena_allocated_memory_get_user_ptr(alloc_memptr);
}
