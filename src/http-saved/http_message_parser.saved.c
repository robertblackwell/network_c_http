...
/// forward declares
static int message_begin_cb(http_parser_t* parser);
static int url_data_cb(http_parser_t* parser, const char* at, size_t length);
static int status_data_cb(http_parser_t* parser, const char* at, size_t length);
static int method_data_cb(http_parser_t* parser, const char* at, size_t length);
static int version_data_cb(http_parser_t* parser, const char* at, size_t length);
static int header_field_data_cb(http_parser_t* parser, const char* at, size_t length);
static int header_value_data_cb(http_parser_t* parser, const char* at, size_t length);
static int chunk_extension_name_cb(http_parser_t* parser, const char* at, size_t length);
static int chunk_extension_value_cb(http_parser_t* parser, const char* at, size_t length);

static int headers_complete_cb(http_parser_t* parser);//, const char* aptr, size_t remainder);
static int body_data_cb(http_parser_t* parser, const char* at, size_t length);
static int message_complete_cb(http_parser_t* parser);

static int on_url_complete_cb(http_parser_t* parser);
static int on_status_complete_cb(http_parser_t* parser);
static int on_method_complete_cb(http_parser_t* parser);
static int on_version_complete_cb(http_parser_t* parser);
static int on_header_field_complete_cb(http_parser_t* parser);
static int on_header_value_complete_cb(http_parser_t* parser);
static int on_chunk_extension_name_complete_cb(http_parser_t* parser);
static int on_chunk_extension_value_complete_cb(http_parser_t* parser);

static int chunk_header_cb(http_parser_t* parser);
static int chunk_complete_cb(http_parser_t* parser);
static int on_reset_cb(http_parser_t* parser);
...
http_errno_t Parser_append_bytes(HttpMessageParserRef this, void *buffer, unsigned length)
{
    RBL_CHECK_TAG(HTTP_PARSER_TAG, this)
    RBL_CHECK_END_TAG(HTTP_PARSER_TAG, this)
    size_t size  = http_parser_execute(this->m_http_parser_ptr,
        this->m_http_parser_settings_ptr,
        (const char*)buffer,
        (int)length);
    return (size == length) ? HPE_OK: this->m_http_parser_ptr->http_errno;
}

http_errno_t http_message_parser_consume(HttpMessageParser *parser, const void* buffer, int length)
{
    RBL_CHECK_TAG(HTTP_PARSER_TAG, parser)
    RBL_CHECK_END_TAG(HTTP_PARSER_TAG, parser)
    char* b = (char*) buffer;
    size_t size = http_parser_execute(parser->m_http_parser_ptr, parser->m_http_parser_settings_ptr, b, length);
    return (size == length) ? HPE_OK: parser->m_http_parser_ptr->http_errno;
}
http_errno_t  http_message_parser_consume_buffer(HttpMessageParser *parser, IOBufferRef iobuffer_ref)
{
    RBL_CHECK_TAG(HTTP_PARSER_TAG, parser)
    RBL_CHECK_END_TAG(HTTP_PARSER_TAG, parser)
    void* bufptr = IOBuffer_data(iobuffer_ref);
    int buflength = IOBuffer_data_len(iobuffer_ref);
    size_t count = http_parser_execute(parser->m_http_parser_ptr, parser->m_http_parser_settings_ptr, bufptr, buflength);
    http_errno_t http_error_no = HPE_OK;
    if (count != buflength) {
        http_error_no = parser->m_http_parser_ptr->http_errno;
    }
    return http_error_no;
}

http_errno_t http_message_parser_get_errno(HttpMessageParser* parser)
{
    RBL_CHECK_TAG(HTTP_PARSER_TAG, parser)
    RBL_CHECK_END_TAG(HTTP_PARSER_TAG, parser)
    http_errno_t x = parser->m_http_parser_ptr->http_errno;
    return x;
}
...
void HttpParser_initialize(HttpMessageParser* this)
{
    ...
    http_parser_settings_t* settings = malloc(sizeof(http_parser_settings_t));
    ...
    this->m_http_parser_ptr = (http_parser_t*)malloc(sizeof(http_parser_t));
...
static
int url_data_cb(http_parser_t* parser, const char* at, size_t length)
{
...
static
int status_data_cb(http_parser_t* parser, const char* at, size_t length)
{
    ...
    http_message_set_status(this->current_message_ptr, this->m_http_parser_ptr->status_code);
...
static
int method_data_cb(http_parser_t* parser, const char* at, size_t length)
{
...
static int headers_complete_cb(http_parser_t* parser) //, const char* aptr, size_t remainder)
{
    ...
    if( Cbuffer_size(this->m_url_buf)  == 0 ) {
        ...
        http_message_set_method(this->current_message_ptr, (http_method_t) parser->method);
...
static
int body_data_cb(http_parser_t* parser, const char* at, size_t length)
{
    HttpMessageParserRef this =  (HttpMessageParserRef)(parser->data);
    RBL_CHECK_TAG(HTTP_PARSER_TAG, this)
...
static
int message_complete_cb(http_parser_t* parser)
{
    HttpMessageParserRef this =  (HttpMessageParserRef)(parser->data);
    RBL_CHECK_TAG(HTTP_PARSER_TAG, this)
...
static
int on_url_complete_cb(http_parser_t* parser)
{
    return 0;
}
static
int on_status_complete_cb(http_parser_t* parser)
{
    return 0;
}
static
int on_method_complete_cb(http_parser_t* parser)
{
    return 0;
}
static
int on_version_complete_cb(http_parser_t* parser)
{
    return 0;
}
static
int on_header_field_complete_cb(http_parser_t* parser)
{
    return 0;
}
static
int on_header_value_complete_cb(http_parser_t* parser)
{
    return 0;
}
static
int on_chunk_extension_name_complete_cb(http_parser_t* parser)
{
    return 0;
}
static
int on_chunk_extension_value_complete_cb(http_parser_t* parser)
{
    return 0;
}

static
int chunk_header_cb(http_parser_t* parser)
{
    HttpMessageParserRef this =  (HttpMessageParserRef)(parser->data);
    return 0;
}
static
int chunk_complete_cb(http_parser_t* parser)
{
    HttpMessageParserRef p =  (HttpMessageParserRef)(parser->data);
    return 0;
}
static
int on_reset_cb(http_parser_t* parser)
{
    HttpMessageParserRef p =  (HttpMessageParserRef)(parser->data);
    return 0;
}
