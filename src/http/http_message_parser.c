...
llhttp_errno_t  http_message_parser_consume_buffer(HttpMessageParser *parser, IOBufferRef iobuffer_ref)
{
    ...
    size_t buflength = IOBuffer_data_len(iobuffer_ref);
...
static int url_data_cb(llhttp_t* parser, const char* at, size_t length)
{
    RBL_LOG_FMT("url_data at: %p length:%d %.*s \n", at, (int)length, (int)length, at);
    HttpMessageParserRef this =  (HttpMessageParserRef)(parser->data);
    RBL_CHECK_TAG(HTTP_PARSER_TAG, this)
    http_message_set_is_request(this->current_message_ptr, true);
    http_message_target_append(this->current_message_ptr, (char*)at, length);
    return 0;
}
static int status_data_cb(llhttp_t* parser, const char* at, size_t length)
{
    RBL_LOG_FMT("status_data at: %p length:%d %.*s \n", at, (int)length, (int)length, at);
    ...
    http_message_reason_append(this->current_message_ptr, (char*)at, length);
    return 0;
}
static int method_data_cb(llhttp_t* parser, const char* at, size_t length)
{
    RBL_LOG_FMT("method_data at: %p length:%d %.*s \n", at, (int)length, (int)length, at);
    return 0;
}
static int version_data_cb(llhttp_t* parser, const char* at, size_t length)
{
    RBL_LOG_FMT("version_data at: %p length:%d %.*s \n", at, (int)length, (int)length, at);
    return 0;
}
static int header_field_data_cb(llhttp_t* parser, const char* at, size_t length)
{
    RBL_LOG_FMT("header_field at: %p length:%d %.*s \n", at, (int)length, (int)length, at);
    HttpMessageParserRef this =  (HttpMessageParserRef)(parser->data);
    RBL_CHECK_TAG(HTTP_PARSER_TAG, this)
    int state = this->m_header_state;
    if( (state == kHEADER_STATE_NOTHING) || (state == kHEADER_STATE_VALUE)) {
        http_message_add_empty_headerline(this->current_message_ptr);
        http_message_last_header_line_append_to_key(this->current_message_ptr, (void*)at, (int)length);
...
static int header_value_data_cb(llhttp_t* parser, const char* at, size_t length)
{
    RBL_LOG_FMT("header_value at: %p length:%d %.*s \n", at, (int)length, (int)length, at);
    HttpMessageParserRef this =  (HttpMessageParserRef)(parser->data);
    RBL_CHECK_TAG(HTTP_PARSER_TAG, this)
    int state = this->m_header_state;
    if( state == kHEADER_STATE_FIELD ) {
        http_message_last_header_line_append_to_value(this->current_message_ptr, (void*)at, (int)length);
...
static int chunk_extension_name_cb(llhttp_t* parser, const char* at, size_t length){return 0;}
static int chunk_extension_value_cb(llhttp_t* parser, const char* at, size_t length){return 0;}
...
static int body_data_cb(llhttp_t* parser, const char* at, size_t length)
{
    ...
    BufferChain_append(chain_ptr, (void*)at, (int)length);
    return 0;
}
static int message_complete_cb(llhttp_t* parser)
{
...
static int on_url_complete_cb(llhttp_t* parser){return 0;}
static int on_status_complete_cb(llhttp_t* parser){return 0;}
static int on_method_complete_cb(llhttp_t* parser){return 0;}
static int on_version_complete_cb(llhttp_t* parser){return 0;}
static int on_header_field_complete_cb(llhttp_t* parser){return 0;}
static int on_header_value_complete_cb(llhttp_t* parser){return 0;}
static int on_chunk_extension_name_complete_cb(llhttp_t* parser){ return 0;}
static int on_chunk_extension_value_complete_cb(llhttp_t* parser){ return 0;}
static int chunk_header_cb(llhttp_t* parser){ return 0;}
static int chunk_complete_cb(llhttp_t* parser){return 0;}
static int on_reset_cb(llhttp_t* parser){return 0;}
