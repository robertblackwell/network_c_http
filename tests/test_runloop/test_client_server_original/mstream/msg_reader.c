...
static void new_message_callback(void* msgstream, NewlineMsgRef new_msg, int error);
static void invoke_read_callback(MsgStreamRef msg_stream_ref, NewlineMsgRef new_msg, int error);
static void try_read(MsgStreamRef msg_stream_ref);

void msg_stream_reader_init(MsgStreamRef ms)
{
    ms->msg_parser_ref = newline_msg_parser_new(new_message_callback, ms);
}
void msg_stream_read(MsgStreamRef msg_stream, MsgReadCallback cb, void* arg)
{
    ...
    if (List_size(msg_stream->input_message_list) != 0) {
        NewlineMsgRef m = List_remove_first(msg_stream->input_message_list);
...
static void tcp_read_callback(void* arg, int error)
{
    ...
    if(error == 0) {
        ...
        newline_msg_parser_consume(msg_stream_ref->msg_parser_ref, msg_stream_ref->input_buffer);//, new_message_callback, msg_stream_ref);
        assert(IOBuffer_data_len(msg_stream_ref->input_buffer) == 0);
        ...
        if (List_size(msg_stream_ref->input_message_list) != 0) {
            NewlineMsgRef m = List_remove_first(msg_stream_ref->input_message_list);
