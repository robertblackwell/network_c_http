...
void tcp_stream_init(TcpStreamRef tcp_stream_ref, RunloopRef rl, int fd)
{
    ...
    tcp_stream_ref->write_state = WRT_STATE_INITIAL;
    tcp_stream_ref->output_buffer = NULL;
    tcp_stream_ref->write_cb = NULL;
    tcp_stream_ref->write_cb_arg = NULL;

    tcp_stream_ref->read_state = RD_STATE_INITIAL;
