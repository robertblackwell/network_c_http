...
static void postable_try_accept(RunloopRef rl, void* arg)
{
    ...
    RBL_CHECK_END_TAG(Ctx_TAG, ctx)
    RunloopEventRef rlevent = ctx->rl_event;
    runloop_listener_register(rlevent, on_accept_ready, ctx);
...
static void handle_new_socket(ListenerCtxRef ctx, int sock)
{
    ...
    ReaderCtxRef rdctx = &(ctx->rdctx);
    reader_ctx_init(rdctx, rl, sock);
    runloop_post(rl, postable_reader_start, rdctx);
    #endif
    // rdctx->stream = runloop_stream_new(rl, sock);
    // ctx->rdctx = rdctx;
    // runloop_stream_arm_read(ctx->conn);
    // char buf[300];
    ...
    if(sock2 > 0) {
       ...
       // listen socket is closed ?? 
        ...
        if(errno_saved == EAGAIN) {
            ...
            runloop_listener_arm(rlevent, on_accept_ready, ctx);
        } else {
