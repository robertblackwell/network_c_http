...
void ReadCtx_init(ReadCtx* this, int my_index, int fd, int max)
{
    ...
    this->read_state = RD_STATE_INITIAL;
    this->id = "READ";
    this->read_count = 0;
    this->read_char_count = 0;
...
int ReaderTable_char_count(ReaderTable* rt)
{
    int count = 0;
    for(int i = 0; i < rt->count; i++) {
        count += rt->ctx_table[i].read_char_count;
    }
    return count;
}
...
static void try_read(ReadCtx* ctx)
{
    ...
    char* s;
    if(nread > 0) {
        ctx->read_char_count += nread;
        buf[nread] = (char)0;
        s = &(buf[0]);
        ctx->read_state = RD_STATE_READY;
        ctx->read_count++;
        RBL_LOG_FMT("try_read nread: %d read_count: %d read_state: %d buf:%s\n", nread, ctx->read_count, ctx->read_state, buf);
...
static void read_clean_termination(ReadCtx* ctx)
{
    ...
    runloop_stream_disarm_read(ctx->reader);
    runloop_stream_free(ctx->reader);
}
static void read_error_termination(ReadCtx* ctx, int err)
{
