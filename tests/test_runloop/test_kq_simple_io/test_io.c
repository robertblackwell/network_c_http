...
int test_io()
{
    ...
    RBL_LOG_FMT("read fd %d write fd: %d\n", pipe_1[0],pipe_1[1]);
    pthread_t rdr_thread;
    pthread_t wrtr_thread;
    async_socket_set_nonblocking(pipe_1[0]);
    int r_wrtr = pthread_create(&wrtr_thread, NULL, writer_thread_func, (void*)(intptr_t)pipe_1[1]);
    int r_rdr = pthread_create(&rdr_thread, NULL, reader_thread_func, (void*)(intptr_t)pipe_1[0]);
