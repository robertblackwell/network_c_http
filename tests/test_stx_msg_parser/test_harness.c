...
test_output_r test_output_new(StxMsgRef msg, int rc)
{
...
void on_message_handler(void* ctx_arg, StxMsgRef msg_ref, int error)
{
    assert(error == 0);
...
int parser_test_run(parser_test_t* this)
{
    ...
    const char* desc = ds_ptr->description;

    StxMsgParserRef parser_ref = stx_msg_parser_new(on_message_handler, (void *) this);
    while(1) {
        int rc;
        ...
        if(bytes_read > 0) {
            ...
            stx_msg_parser_consume(parser_ref, iob);
        } else if(bytes_read == 0) {
            // simulate eof
            printf("WPT_run status == 0 io error bytes_read: %d\n", bytes_read);
//            List_add_back(this->m_results, test_output_new(NULL, bytes_read));
    ...
    printf("Return from verify %d  testcase : %s\n", r, desc);
